var ModuleFactory = (() => {
	...
	return async function (moduleArg = {}) {
		...
		var findEventTarget = (target) => {
			...
			else if (typeof target == "string") return typeof document != "undefined" ? document.getElementById(target) : null;
			return target;
		};
		...
		var WebGPU = {
			...
			makeColor: (ptr) => ({ r: HEAPF64[ptr >> 3], g: HEAPF64[(ptr + 8) >> 3], b: HEAPF64[(ptr + 16) >> 3], a: HEAPF64[(ptr + 24) >> 3] }),
			makeExtent3D: (ptr) => ({ width: HEAPU32[ptr >> 2], height: HEAPU32[(ptr + 4) >> 2], depthOrArrayLayers: HEAPU32[(ptr + 8) >> 2] }),
			...
			makePassTimestampWrites: (ptr) => {
				...
				return { querySet: WebGPU.getJsObject(HEAPU32[(ptr + 4) >> 2]), beginningOfPassWriteIndex: HEAPU32[(ptr + 8) >> 2], endOfPassWriteIndex: HEAPU32[(ptr + 12) >> 2] };
			},
			...
			makeRenderPipelineDesc: (descriptor) => {
				...
				function makeColorState(csPtr) {
					...
					return format ? { format, blend: makeBlendState(HEAPU32[(csPtr + 8) >> 2]), writeMask: HEAPU32[(csPtr + 16) >> 2] } : undefined;
				}
				...
				function makeVertexAttribute(vaPtr) {
					return { format: WebGPU.VertexFormat[HEAP32[(vaPtr + 4) >> 2]], offset: readI53FromI64(vaPtr + 8), shaderLocation: HEAPU32[(vaPtr + 16) >> 2] };
				}
				...
				function makeVertexBuffer(vbPtr) {
					...
					return { arrayStride: readI53FromI64(vbPtr + 8), stepMode, attributes: makeVertexAttributes(attributeCount, HEAPU32[(vbPtr + 20) >> 2]) };
				}
				...
				function makeMultisampleState(msPtr) {
					...
					return { count: HEAPU32[(msPtr + 4) >> 2], mask: HEAPU32[(msPtr + 8) >> 2], alphaToCoverageEnabled: !!HEAPU32[(msPtr + 12) >> 2] };
				}
		...
		var _wgpuCommandEncoderBeginComputePass = (encoderPtr, descriptor) => {
			var desc;
			if (descriptor) {
				desc = { label: WebGPU.makeStringFromOptionalStringView(descriptor + 4), timestampWrites: WebGPU.makePassTimestampWrites(HEAPU32[(descriptor + 12) >> 2]) };
			}
		...
		var _wgpuDeviceCreateBindGroupLayout = (devicePtr, descriptor) => {
			function makeBufferEntry(substructPtr) {
				...
				return { type: WebGPU.BufferBindingType[typeInt], hasDynamicOffset: !!HEAPU32[(substructPtr + 8) >> 2], minBindingSize: readI53FromI64(substructPtr + 16) };
			}
			...
			var desc = { label: WebGPU.makeStringFromOptionalStringView(descriptor + 4), entries: makeEntries(HEAPU32[(descriptor + 12) >> 2], HEAPU32[(descriptor + 16) >> 2]) };
			var device = WebGPU.getJsObject(devicePtr);
			var ptr = _emwgpuCreateBindGroupLayout(0);
		...
		var _wgpuDeviceCreatePipelineLayout = (devicePtr, descriptor) => {
			...
			var desc = { label: WebGPU.makeStringFromOptionalStringView(descriptor + 4), bindGroupLayouts: bgls };
			var device = WebGPU.getJsObject(devicePtr);
		...
		var ASM_CONSTS = {
			...
			1486338: ($0, $1, $2, $3, $4) => {
				...
				const width = $3;
				const height = $4;
				device.queue.copyExternalImageToTexture({ source: drawable }, { texture }, [width, height]);
			},
			1486597: ($0, $1, $2, $3) => {
			...
			1487560: () => typeof HTMLCanvasElement !== "undefined",
			1487615: () => !!Module["preinitializedWebGPUDevice"],
			1487666: () => {
		...
		function BeginGlQueryTiming(calc_name, num_repetitions) {
			...
			Module.WEBGL_SHADER_CALC_METRICS[UTF8ToString(calc_name)] = { query, repetitions: num_repetitions };
		...
		function JsOnWebGLTextureListener(output_stream_name, name, width, height, timestamp_ms) {
			Module._wrapSimpleListenerOutput(output_stream_name, { data: GL.textures[name], width, height }, timestamp_ms);
		}
		...
		function JsOnWebGLTextureVectorListener(output_stream_name, name, width, height, timestamp_ms) {
			Module._wrapSimpleListenerOutput(output_stream_name, { data: GL.textures[name], width, height }, false, timestamp_ms);
		}
		...
		function assignWasmExports(wasmExports) {
			_free = Module["_free"] = wasmExports["ld"];
			_malloc = Module["_malloc"] = wasmExports["md"];
			_wgpuDeviceAddRef = wasmExports["nd"];
			_addBoundTextureAsImageToStream = Module["_addBoundTextureAsImageToStream"] = wasmExports["od"];
			_attachImageListener = Module["_attachImageListener"] = wasmExports["pd"];
			_attachImageVectorListener = Module["_attachImageVectorListener"] = wasmExports["qd"];
			_registerModelResourcesGraphService = Module["_registerModelResourcesGraphService"] = wasmExports["rd"];
			_bindTextureToStream = Module["_bindTextureToStream"] = wasmExports["sd"];
			_addBoundTextureToStream = Module["_addBoundTextureToStream"] = wasmExports["td"];
			_addDoubleToInputStream = Module["_addDoubleToInputStream"] = wasmExports["ud"];
			_addFloatToInputStream = Module["_addFloatToInputStream"] = wasmExports["vd"];
			_addBoolToInputStream = Module["_addBoolToInputStream"] = wasmExports["wd"];
			_addIntToInputStream = Module["_addIntToInputStream"] = wasmExports["xd"];
			_addUintToInputStream = Module["_addUintToInputStream"] = wasmExports["yd"];
			_addStringToInputStream = Module["_addStringToInputStream"] = wasmExports["zd"];
			_addRawDataSpanToInputStream = Module["_addRawDataSpanToInputStream"] = wasmExports["Ad"];
			_allocateBoolVector = Module["_allocateBoolVector"] = wasmExports["Bd"];
			_allocateFloatVector = Module["_allocateFloatVector"] = wasmExports["Cd"];
			_allocateDoubleVector = Module["_allocateDoubleVector"] = wasmExports["Dd"];
			_allocateIntVector = Module["_allocateIntVector"] = wasmExports["Ed"];
			_allocateUintVector = Module["_allocateUintVector"] = wasmExports["Fd"];
			_allocateStringVector = Module["_allocateStringVector"] = wasmExports["Gd"];
			_addBoolVectorEntry = Module["_addBoolVectorEntry"] = wasmExports["Hd"];
			_addFloatVectorEntry = Module["_addFloatVectorEntry"] = wasmExports["Id"];
			_addDoubleVectorEntry = Module["_addDoubleVectorEntry"] = wasmExports["Jd"];
			_addIntVectorEntry = Module["_addIntVectorEntry"] = wasmExports["Kd"];
			_addUintVectorEntry = Module["_addUintVectorEntry"] = wasmExports["Ld"];
			_addStringVectorEntry = Module["_addStringVectorEntry"] = wasmExports["Md"];
			_addBoolVectorToInputStream = Module["_addBoolVectorToInputStream"] = wasmExports["Nd"];
			_addFloatVectorToInputStream = Module["_addFloatVectorToInputStream"] = wasmExports["Od"];
			_addDoubleVectorToInputStream = Module["_addDoubleVectorToInputStream"] = wasmExports["Pd"];
			_addIntVectorToInputStream = Module["_addIntVectorToInputStream"] = wasmExports["Qd"];
			_addUintVectorToInputStream = Module["_addUintVectorToInputStream"] = wasmExports["Rd"];
			_addStringVectorToInputStream = Module["_addStringVectorToInputStream"] = wasmExports["Sd"];
			_addFlatHashMapToInputStream = Module["_addFlatHashMapToInputStream"] = wasmExports["Td"];
			_addProtoToInputStream = Module["_addProtoToInputStream"] = wasmExports["Ud"];
			_addEmptyPacketToInputStream = Module["_addEmptyPacketToInputStream"] = wasmExports["Vd"];
			_addBoolToInputSidePacket = Module["_addBoolToInputSidePacket"] = wasmExports["Wd"];
			_addDoubleToInputSidePacket = Module["_addDoubleToInputSidePacket"] = wasmExports["Xd"];
			_addFloatToInputSidePacket = Module["_addFloatToInputSidePacket"] = wasmExports["Yd"];
			_addIntToInputSidePacket = Module["_addIntToInputSidePacket"] = wasmExports["Zd"];
			_addUintToInputSidePacket = Module["_addUintToInputSidePacket"] = wasmExports["_d"];
			_addStringToInputSidePacket = Module["_addStringToInputSidePacket"] = wasmExports["$d"];
			_addRawDataSpanToInputSidePacket = Module["_addRawDataSpanToInputSidePacket"] = wasmExports["ae"];
			_addProtoToInputSidePacket = Module["_addProtoToInputSidePacket"] = wasmExports["be"];
			_addBoolVectorToInputSidePacket = Module["_addBoolVectorToInputSidePacket"] = wasmExports["ce"];
			_addDoubleVectorToInputSidePacket = Module["_addDoubleVectorToInputSidePacket"] = wasmExports["de"];
			_addFloatVectorToInputSidePacket = Module["_addFloatVectorToInputSidePacket"] = wasmExports["ee"];
			_addIntVectorToInputSidePacket = Module["_addIntVectorToInputSidePacket"] = wasmExports["fe"];
			_addUintVectorToInputSidePacket = Module["_addUintVectorToInputSidePacket"] = wasmExports["ge"];
			_addStringVectorToInputSidePacket = Module["_addStringVectorToInputSidePacket"] = wasmExports["he"];
			_attachBoolListener = Module["_attachBoolListener"] = wasmExports["ie"];
			_attachBoolVectorListener = Module["_attachBoolVectorListener"] = wasmExports["je"];
			_attachDoubleListener = Module["_attachDoubleListener"] = wasmExports["ke"];
			_attachDoubleVectorListener = Module["_attachDoubleVectorListener"] = wasmExports["le"];
			_attachFloatListener = Module["_attachFloatListener"] = wasmExports["me"];
			_attachFloatVectorListener = Module["_attachFloatVectorListener"] = wasmExports["ne"];
			_attachIntListener = Module["_attachIntListener"] = wasmExports["oe"];
			_attachIntVectorListener = Module["_attachIntVectorListener"] = wasmExports["pe"];
			_attachUintListener = Module["_attachUintListener"] = wasmExports["qe"];
			_attachUintVectorListener = Module["_attachUintVectorListener"] = wasmExports["re"];
			_attachStringListener = Module["_attachStringListener"] = wasmExports["se"];
			_attachStringVectorListener = Module["_attachStringVectorListener"] = wasmExports["te"];
			_attachProtoListener = Module["_attachProtoListener"] = wasmExports["ue"];
			_attachProtoVectorListener = Module["_attachProtoVectorListener"] = wasmExports["ve"];
			_getGraphConfig = Module["_getGraphConfig"] = wasmExports["we"];
			___getTypeName = wasmExports["xe"];
			_emwgpuCreateBindGroup = wasmExports["ye"];
			_emwgpuCreateBindGroupLayout = wasmExports["ze"];
			_emwgpuCreateCommandBuffer = wasmExports["Ae"];
			_emwgpuCreateCommandEncoder = wasmExports["Be"];
			_emwgpuCreateComputePassEncoder = wasmExports["Ce"];
			_emwgpuCreateComputePipeline = wasmExports["De"];
			_emwgpuCreateExternalTexture = wasmExports["Ee"];
			_emwgpuCreatePipelineLayout = wasmExports["Fe"];
			_emwgpuCreateQuerySet = wasmExports["Ge"];
			_emwgpuCreateRenderBundle = wasmExports["He"];
			_emwgpuCreateRenderBundleEncoder = wasmExports["Ie"];
			_emwgpuCreateRenderPassEncoder = wasmExports["Je"];
			_emwgpuCreateRenderPipeline = wasmExports["Ke"];
			_emwgpuCreateSampler = wasmExports["Le"];
			_emwgpuCreateSurface = wasmExports["Me"];
			_emwgpuCreateTexture = wasmExports["Ne"];
			_emwgpuCreateTextureView = wasmExports["Oe"];
			_emwgpuCreateAdapter = wasmExports["Pe"];
			_emwgpuCreateBuffer = wasmExports["Qe"];
			_emwgpuCreateDevice = wasmExports["Re"];
			_emwgpuCreateQueue = wasmExports["Se"];
			_emwgpuCreateShaderModule = wasmExports["Te"];
			_emwgpuOnCreateComputePipelineCompleted = wasmExports["Ue"];
			_emwgpuOnCreateRenderPipelineCompleted = wasmExports["Ve"];
			_clearSubgraphs = Module["_clearSubgraphs"] = wasmExports["We"];
			_pushBinarySubgraph = Module["_pushBinarySubgraph"] = wasmExports["Xe"];
			_pushTextSubgraph = Module["_pushTextSubgraph"] = wasmExports["Ye"];
			_changeBinaryGraph = Module["_changeBinaryGraph"] = wasmExports["Ze"];
			_changeTextGraph = Module["_changeTextGraph"] = wasmExports["_e"];
			_processGl = Module["_processGl"] = wasmExports["$e"];
			_process = Module["_process"] = wasmExports["af"];
			_bindTextureToCanvas = Module["_bindTextureToCanvas"] = wasmExports["bf"];
			_requestShaderRefreshOnGraphChange = Module["_requestShaderRefreshOnGraphChange"] = wasmExports["cf"];
			_waitUntilIdle = Module["_waitUntilIdle"] = wasmExports["df"];
			_closeGraph = Module["_closeGraph"] = wasmExports["ef"];
			_setAutoRenderToScreen = Module["_setAutoRenderToScreen"] = wasmExports["ff"];
			...
			__indirect_function_table = wasmTable = wasmExports["kd"];
		}
		...
		var wasmImports = {
			hd: BeginGlQueryTiming,
			gd: EndGlQueryTiming,
			fd: GetAdapterArchitecture,
			ed: GetAdapterDescription,
			dd: GetAdapterDeviceName,
			cd: GetAdapterVendor,
			bd: JsOnEmptyPacketListener,
			ad: JsOnFloat32ArrayImageListener,
			$c: JsOnFloat32ArrayImageVectorListener,
			pb: JsOnSimpleListenerBinaryArray,
			_c: JsOnSimpleListenerBool,
			Zc: JsOnSimpleListenerDouble,
			Yc: JsOnSimpleListenerFloat,
			Xc: JsOnSimpleListenerInt,
			Wc: JsOnSimpleListenerString,
			Vc: JsOnSimpleListenerUint,
			Uc: JsOnUint8ArrayImageListener,
			Tc: JsOnUint8ArrayImageVectorListener,
			O: JsOnVectorFinishedListener,
			Sc: JsOnVectorListenerBool,
			Rc: JsOnVectorListenerDouble,
			Qc: JsOnVectorListenerFloat,
			Pc: JsOnVectorListenerInt,
			Oc: JsOnVectorListenerProto,
			Nc: JsOnVectorListenerString,
			Mc: JsOnVectorListenerUint,
			Lc: JsOnWebGLTextureListener,
			Kc: JsOnWebGLTextureVectorListener,
			Pa: JsWrapErrorListener,
			ob: JsWrapImageConverter,
			u: JsWrapSimpleListeners,
			nb: UseBottomLeftGpuOrigin,
			yb: __asyncjs__mediapipe_map_buffer_jspi,
			o: ___cxa_throw,
			Jc: ___syscall_dup,
			Ic: ___syscall_faccessat,
			mb: ___syscall_fcntl64,
			Hc: ___syscall_fstat64,
			Mb: ___syscall_ftruncate64,
			Gc: ___syscall_ioctl,
			Fc: ___syscall_lstat64,
			Ec: ___syscall_newfstatat,
			lb: ___syscall_openat,
			Dc: ___syscall_stat64,
			yc: __abort_js,
			Jb: __embind_register_bigint,
			xc: __embind_register_bool,
			wc: __embind_register_emval,
			jb: __embind_register_float,
			I: __embind_register_integer,
			r: __embind_register_memory_view,
			vc: __embind_register_std_string,
			Ma: __embind_register_std_wstring,
			uc: __embind_register_void,
			$: __emval_create_invoker,
			q: __emval_decref,
			La: __emval_get_global,
			ib: __emval_get_property,
			ja: __emval_incref,
			Ka: __emval_instanceof,
			_: __emval_invoke,
			ua: __emval_new_cstring,
			Z: __emval_run_destructors,
			hb: __emval_set_property,
			tc: __emval_typeof,
			Ib: __gmtime_js,
			Hb: __localtime_js,
			Gb: __mktime_js,
			...
			sc: __tzset_js,
			Lb: _clock_time_get,
			rc: custom_emscripten_dbgn,
			Y: _emscripten_asm_const_int,
			gb: _emscripten_asm_const_ptr,
			Ja: _emscripten_errn,
			...
			fb: _emscripten_stack_snapshot,
			mc: _emscripten_stack_unwind_buffer,
			lc: _emscripten_webgl_create_context,
			kc: _emscripten_webgl_destroy_context,
			jc: _emscripten_webgl_get_context_attributes,
			ha: _emscripten_webgl_get_current_context,
			ic: _emscripten_webgl_make_context_current,
			Q: _emscripten_webgpu_get_device,
			hc: _emwgpuBufferDestroy,
			gc: _emwgpuBufferGetMappedRange,
			fc: _emwgpuBufferUnmap,
			x: _emwgpuDelete,
			ec: _emwgpuDeviceCreateBuffer,
			Db: _emwgpuDeviceCreateComputePipelineAsync,
			Cb: _emwgpuDeviceCreateRenderPipelineAsync,
			dc: _emwgpuDeviceCreateShaderModule,
			cc: _emwgpuDeviceDestroy,
			bc: _emwgpuWaitAny,
			Cc: _environ_get,
			Bc: _environ_sizes_get,
			eb: _exit,
			Oa: _fd_close,
			kb: _fd_read,
			Kb: _fd_seek,
			...
			b: _glActiveTexture,
			ta: _glAttachShader,
			ac: _glBindAttribLocation,
			c: _glBindBuffer,
			db: _glBindBufferBase,
			...
			F: _glGenTextures,
			B: _glGenVertexArrays,
			Xa: _glGetAttribLocation,
			ea: _glGetError,
			Zb: _glGetFloatv,
			w: _glGetIntegerv,
			Yb: _glGetProgramiv,
			Xb: _glGetShaderInfoLog,
			Wb: _glGetShaderiv,
			N: _glGetString,
			Vb: _glGetUniformBlockIndex,
			...
			Ob: _glUniformMatrix2fv,
			Nb: _glUniformMatrix3fv,
			Ha: _glUniformMatrix4fv,
			g: _glUseProgram,
			k: _glVertexAttribPointer,
			S: _glViewport,
			Ga: hardware_concurrency,
			Bb: mediapipe_create_utility_canvas2d,
			Ab: _mediapipe_find_canvas_event_target,
			zb: mediapipe_import_external_texture,
			xb: _mediapipe_webgl_tex_image_drawable,
			Ac: _proc_exit,
			zc: _random_get,
			Fa: _wgpuCommandEncoderBeginComputePass,
			Ea: _wgpuCommandEncoderBeginRenderPass,
			wb: _wgpuCommandEncoderCopyBufferToTexture,
			vb: _wgpuCommandEncoderCopyTextureToBuffer,
			ub: _wgpuCommandEncoderCopyTextureToTexture,
			L: _wgpuCommandEncoderFinish,
			Da: _wgpuComputePassEncoderDispatchWorkgroups,
			Ca: _wgpuComputePassEncoderEnd,
			Ba: _wgpuComputePassEncoderSetBindGroup,
			Aa: _wgpuComputePassEncoderSetPipeline,
			za: _wgpuComputePipelineGetBindGroupLayout,
			ca: _wgpuDeviceCreateBindGroup,
			tb: _wgpuDeviceCreateBindGroupLayout,
			K: _wgpuDeviceCreateCommandEncoder,
			sb: _wgpuDeviceCreateComputePipeline,
			rb: _wgpuDeviceCreatePipelineLayout,
			Sa: _wgpuDeviceCreateRenderPipeline,
			R: _wgpuDeviceCreateSampler,
			ba: _wgpuDeviceCreateTexture,
			J: _wgpuQueueSubmit,
			ka: _wgpuQueueWriteBuffer,
			qb: _wgpuQueueWriteTexture,
			...
			Ra: _wgpuRenderPipelineGetBindGroupLayout,
			z: _wgpuTextureCreateView,
			Qa: _wgpuTextureDestroy,
