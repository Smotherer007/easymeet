...
function renderChatMessage(m, myNick) {
  ...
  return content ? `<div class="chat__msg${selfClass}"><div class="chat__msg-header"><span class="chat__msg-nick">${escapeHtml(m.nick ?? '?')}</span><span class="chat__msg-time">${formatTime(m.ts)}</span></div><div class="chat__msg-body">${content}</div></div>` : '';
}
        ...
        <div class="chat__input-wrap">
          ...
          <div class="chat__input-row">
            <div class="emoji-picker" id="emoji-picker" hidden>
              ...
              <div class="emoji-picker__grid" id="emoji-grid">${EMOJI_DATA.slice(0, 500).map(([e]) => `<button type="button" class="emoji-picker__btn" data-emoji="${e}">${e}</button>`).join('')}</div>
            </div>
...
export function renderEffectTilesFirst(backgroundImages, backgroundEffect) {
  return (backgroundImages || []).slice(0, 3).map((bg) => renderEffectTile(bg, backgroundEffect)).join('');
}
