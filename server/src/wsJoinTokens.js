...
/** @type {Map<string, { roomId: string; peerId: string; expires: number }>} */
 ...
 */
export function issueHandshakeToken(roomIdNorm, peerId) {
	sweep();
	const wsToken = crypto.randomBytes(32).toString("hex");
	pending.set(wsToken, { roomId: roomIdNorm, peerId, expires: Date.now() + TTL_MS });
	return wsToken;
}
...
export function consumeHandshakeToken(token) {
	...
	return { roomId: rec.roomId, peerId: rec.peerId };
}
