 ...
 *
 * Path in **EASYMEET_PERSISTENT_ROOMS** (relative to process cwd or absolute).
 */
import fs from 'fs';
...
async function applyRoomListToMap(list, roomsMap, sourceLabel) {
  if (!list || list.length === 0) {
    console.warn('[easymeet] persistent-rooms: no entries in', sourceLabel);
  ...
  for (const entry of list) {
    ...
    if (!roomId) {
      console.warn('[easymeet] persistent-rooms: skipped (invalid id):', entry.id ?? entry.roomId);
      continue;
    }
    if (seen.has(roomId)) {
      console.warn('[easymeet] persistent-rooms: duplicate id, later entry overwrites:', roomId);
    }
    seen.add(roomId);
  ...
  console.log(`[easymeet] persistent-rooms: ${count} room(s) loaded from ${sourceLabel} (TTL excluded)`);
...
export async function applyPersistentRooms(roomsMap) {
  ...
  if (!configPath) {
    console.log(
      '[easymeet] persistent-rooms: optional — set EASYMEET_PERSISTENT_ROOMS in .env (path to JSON), see config/persistent-rooms.example.json'
    );
    return { loaded: 0, path: '(not set)', skipped: true };
  }

  if (!fs.existsSync(configPath)) {
    console.log('[easymeet] persistent-rooms: file missing:', configPath);
  ...
  try {
    ...
    console.error('[easymeet] persistent-rooms: invalid JSON:', configPath, e?.message || e);
    return { loaded: 0, path: configPath, error: true };
  }

  const list = parseRoomsArray(raw);
  if (!list || list.length === 0) {
    console.warn('[easymeet] persistent-rooms: no entries in', configPath);
